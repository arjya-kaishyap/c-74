...
export default class SearchScreen extends Component {
  ...
  fetchMoreTransactions = async text => {
    ...
    if (enteredText[0] === "B") {
      const query = await db.collection("transactions").where("bookId", "==", text).startAfter(lastVisibleTransaction)
      .limit(10)
      .get()
      query.docs.map(doc => {
        this.setState({
      ...
      const query = await db.collection("transactions").where("studentId", "==", text).startAfter(lastVisibleTransaction)
      .limit(10)
      .get()
      query.docs.map(doc => {
        this.setState({
